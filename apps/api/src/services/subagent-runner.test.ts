...
describe("子代理任务编排(看板)", () => {
	it("消费看板:pending 卡片并行执行后回填 done 与结果", async () => {
		const card = createKanbanTask({ tenantId: "t1", title: "查价格", goal: "查旗舰版价格" });
	...
	it("子代理可用工具白名单过滤:只注入允许的工具", async () => {
		createKanbanTask({ tenantId: "t1", title: "计算", goal: "算 1+1", tools: ["computer"] });
		const executed = await consumeKanbanSubagents({ db, streamFn: textStreamFn("结果是 2") }, "t1", { limit: 2 });
		expect(executed[0].status).toBe("done");
	});

	it("执行异常:卡片回填 error", async () => {
		createKanbanTask({ tenantId: "t1", title: "坏任务", goal: "触发异常" });
	...
	it("多任务并行执行,已完成任务不再执行", async () => {
		createKanbanTask({ tenantId: "t1", title: "A", goal: "任务A" });
		createKanbanTask({ tenantId: "t1", title: "B", goal: "任务B" });
		expect(listKanbanCards("t1", "pending")).toHaveLength(2);
		...
		expect(listKanbanCards("t1", "done")).toHaveLength(2);
	});

	it("子代理不允许编排类工具(subagents/kanban 不注入)", async () => {
		createKanbanTask({ tenantId: "t1", title: "协作", goal: "尝试派生", tools: ["subagents", "kanban", "computer"] });
