...
export function createAutomationJob(db: DatabaseSync, input: AutomationJobInput): AutomationJob {
	const id = randomUUID();
	db.prepare(
		"INSERT INTO automation_jobs (id, tenant_id, name, schedule_type, weekday, schedule_time, description, action_json, enabled) VALUES (?,?,?,?,?,?,?,?,?)",
	).run(id, input.tenantId, input.name, input.scheduleType ?? "daily", input.weekday ?? null, input.scheduleTime, input.description ?? null, JSON.stringify(input.action ?? {}), input.enabled === false ? 0 : 1);
...
export function updateAutomationJob(
	...
	db.prepare(
		`UPDATE automation_jobs SET name = ?, schedule_type = ?, weekday = ?, schedule_time = ?, description = ?, action_json = ?, enabled = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = ?`,
	).run(
...
function mapRow(row: Record<string, unknown>): AutomationJob {
	return {
		...
		scheduleType: String(row.schedule_type) as "daily" | "weekly",
