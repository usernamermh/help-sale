...
export function execute(ctx: ToolContext, params: any) {
	...
	const lines = ["| 客户标识 | 姓名 | 电话 | 阶段 | 意向车型 | 最近分析 | 会话数 |", "| --- | --- | --- | --- | --- | --- | --- |"];
	for (const r of rows as any[]) {
		lines.push(`| ${r.key} | ${r.name ?? "—"} | ${r.phone ?? "—"} | ${r.stage ?? "—"} | ${r.intendedVehicles?.join("、") ?? "—"} | ${r.lastAnalysisAt?.slice(0, 10) ?? "—"} | ${r.conversationCount} |`);
	}
